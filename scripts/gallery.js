...
const files = fs
  ...
  .map(
    (filename) =>
      `![俊宁的图床](https://cdn.jsdelivr.net/gh/youngjuning/images@main/${filename})`
  );
...
description: 俊宁的图床，用于存放一些图片，方便在其他地方使用。
type: galleryGroup
tags:
  - 图床
  - 俊宁的图床
...
> [俊宁的图床](https://github.com/youngjuning/images)，用于存放一些图片，方便在其他地方使用。
`;
result += '\n{% gallery true %}\n';
for (file of files) {
  result += file + '\n';
